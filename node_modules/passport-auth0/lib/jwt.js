...
/**
 * Verifier for ID Tokens following OIDC spec.
...
var verify = (token, options) => {
  ...
  if (payload.iss !== options.iss) {
    throw new Error(`Issuer (iss) claim mismatch in the ID token; expected "${options.iss}", found "${payload.iss}"`);
  }
  ...
  if (Array.isArray(payload.aud) && !payload.aud.includes(options.aud)) {
    throw new Error(`Audience (aud) claim mismatch in the ID token; expected "${options.aud}" but was not one of "${payload.aud.join(', ')}"`);
  }
  else if (typeof payload.aud === 'string' && payload.aud !== options.aud) {
    throw new Error(`Audience (aud) claim mismatch in the ID token; expected "${options.aud}" but found "${payload.aud}"`);
  }
  ...
  if (now > expTime) {
    throw new Error(`Expiration Time (exp) claim error in the ID token; current time (${now}) is after expiration time (${expTime})`);
  }
  ...
  if (options.nonce) {
    ...
    if (payload.nonce !== options.nonce) {
      throw new Error(`Nonce (nonce) claim mismatch in the ID token; expected "${options.nonce}", found "${payload.nonce}"`);
    }
  }
  ...
  if (Array.isArray(payload.aud) && payload.aud.length > 1) {
    ...
    if (payload.azp !== options.aud) {
      throw new Error(`Authorized Party (azp) claim mismatch in the ID token; expected "${options.aud}", found "${payload.azp}"`);
    }
  }
